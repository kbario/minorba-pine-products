...
export const reducePostPricelistData =
  ...
  (
    ...
    acc[PostVariants.Regular].push(makeRegular(size, props.isWholesale));

    if (size.hasGrade2) {
      acc[PostVariants.Regular].push(makeGrade2(size, props.isWholesale));
    }
    if (size.hasPointed && !props.isWholesale) {
      acc[PostVariants.Pointed].push(makePointedPost(size, props.isWholesale));
    }
    if (size.hasSheepYard) {
      acc[PostVariants.SheepYard].push(makeSheepYard(size, props.isWholesale));
    }
    if (size.hasCattleYard) {
      acc[PostVariants.CattleYard].push(
        makeCattleYard(size, props.isWholesale),
      );
    }
    if (size.hasHorseRails) {
      acc[PostVariants.HorsePosts].push(
        makeHorseRails(size, props.isWholesale),
      );
    }
    if (size.hasHalfPost) {
      acc[PostVariants.HalfPosts].push(makeHalfPosts(size, props.isWholesale));
    }
    return acc;
  };
...
export const reduceSawnPricelistData =
  ...
  (acc: SawnPricelistData, size: SawnConfig): Required<SawnPricelistData> => {
    ...
    acc[size.type].push(makeSawn(size, props.isWholesale));
    return acc;
  };
