...
const BreadcrumbList: Component<ComponentProps<"ol">> = (props) => {
  ...
  return (
    <ol
      class={cn(
        "text-on-surface-light flex flex-wrap items-center gap-1.5 break-words text-sm sm:gap-2.5",
        local.class,
      )}
      {...others}
    />
...
const BreadcrumbLink = <T extends ValidComponent = "a">(
  ...
  return (
    <BreadcrumbPrimitive.Link
      class={cn(
        "hover:text-on-surface-strong data-[current]:text-on-surface-strong transition-colors data-[current]:font-normal",
        local.class,
      )}
      {...others}
    />
